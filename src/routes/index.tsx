...
function Dashboard() {
  ...
  const cards = [
    { label: "Inventory Items", value: stats?.itemsCount ?? 0, sub: `${stats?.totalUnits ?? 0} total units`, icon: Package, to: "/inventory" as const, accent: "bg-primary-soft text-primary" },
    { label: "Active Pallets", value: stats?.palletsCount ?? 0, sub: "Across all shipments", icon: Layers, to: "/pallets" as const, accent: "bg-accent text-accent-foreground" },
    { label: "Containers Preparing", value: stats?.containersReady ?? 0, sub: "Awaiting shipment", icon: ContainerIcon, to: "/shipments" as const, accent: "bg-primary-soft text-primary" },
    { label: "Hazmat Items", value: stats?.hazmatCount ?? 0, sub: "Flagged for handling", icon: AlertTriangle, to: "/inventory" as const, accent: "bg-warning/20 text-warning-foreground" },
  ];

  return (
    <AppShell title="Dashboard" subtitle="Operations overview — Medical Supplies Network Inc.">
